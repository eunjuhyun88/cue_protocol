// ============================================================================
// 🧠 실제 Personal CUE 추출 서비스 (Mock 제거)
// 경로: backend/src/services/ai/RealPersonalCueExtractor.ts
...
export class RealPersonalCueExtractor {
  ...
  public static getInstance(): RealPersonalCueExtractor {
    if (!RealPersonalCueExtractor.instance) {
      RealPersonalCueExtractor.instance = new RealPersonalCueExtractor();
    }
    return RealPersonalCueExtractor.instance;
  }
  ...
  private extractCommunicationStyle(context: ChatContext): ExtractedCue[] {
    ...
    // 정중함 수준 분석
    const politenessIndicators = ['부탁', '죄송', '감사', '고마워', '도움', '양해'];
  ...
  private extractTopicInterests(context: ChatContext): ExtractedCue[] {
    ...
    const topics = {
      ...
      'ai_machine_learning': {
        keywords: ['ai', '인공지능', 'machine learning', '머신러닝', 'deep learning', 'ml'],
        weight: 1.3
      },
      'data_science': {
  ...
  private isPositiveContext(text: string, keyword: string): boolean {
    ...
    const positiveWords = ['좋아', '선호', '추천', '좋은', '최고', '훌륭', '좋아요'];
    const negativeWords = ['싫어', '안좋아', '별로', '나쁜', '문제', '어려워'];
  ...
  private extractInterestContext(text: string, keyword: string): string {
    const context = this.extractSentenceContext(text, keyword);
    if (context.includes('배우고')) return 'learning_intent';
    if (context.includes('사용하고')) return 'usage_intent';
    if (context.includes('알고싶어')) return 'curiosity';
    if (context.includes('질문')) return 'inquiry';
  ...
  private categorizeQuestionType(message: string): string {
    if (message.includes('어떻게')) return 'how_to';
    if (message.includes('왜')) return 'why';
    if (message.includes('무엇')) return 'what';
    if (message.includes('언제')) return 'when';
    if (message.includes('어디')) return 'where';
  ...
  private async findExistingCue(
    ...
    try {
      // Personal Cues 조회해서 중복 확인
      const cues = await this.db.getPersonalCues(userId, 1000);
      return cues.find(cue => 
        cue.cue_key === cueKey && cue.cue_type === cueType
      );
    } catch (error) {
  ...
  private async updateExistingCue(
    ...
    try {
      ...
      const updatedCueData = {
        id: existingCue.id,
        user_id: existingCue.user_id,
        cue_key: existingCue.cue_key,
        cue_type: existingCue.cue_type,
        cue_category: existingCue.cue_category,
        cue_data: {
        ...
        confidence_metrics: {
          ...
          reliability_score: Math.min(1.0, updatedConfidence + 0.1)
        },
        temporal_data: {
          ...existingCue.temporal_data,
          usage_frequency: updatedFrequency,
          last_reinforced: new Date().toISOString(),
          reinforcement_count: (existingCue.temporal_data?.reinforcement_count || 0) + 1
        },
        updated_at: new Date().toISOString()
      };

      // 실제 DB 업데이트는 여기서 구현 필요
      // await this.db.updatePersonalCue(updatedCueData);
